...
/**
 * Interface for the experimental User-Agent Client Hints API.
 * This is often not included in standard lib.dom.d.ts.
 */
interface NavigatorUAData {
    mobile: boolean;
    brands: Array<{ brand: string; version: string }>;
    getHighEntropyValues(hints: string[]): Promise<{ platform: string; platformVersion: string; [key: string]: any }>;
}

/**
 * Extending the standard Navigator interface to include experimental properties.
 */
...
export const getDeviceMemory = (): number | null =>
    safeWindowAccess(() => (navigator as ExtendedNavigator).deviceMemory ?? null, null);

export const getScreenHeight = (): number | null =>
    safeWindowAccess(() => window.screen.height, null);

export const getScreenWidth = (): number | null =>
    safeWindowAccess(() => window.screen.width, null);

export const getUserAgent = (): string =>
    safeWindowAccess(() => navigator.userAgent, '');

export const getHardwareConcurrency = (): number | null => // CPU threads that I have access to 
    safeWindowAccess(() => navigator.hardwareConcurrency, null);

export const getConnection = (): ExtendedNavigator['connection'] | null =>
    safeWindowAccess(() => (navigator as ExtendedNavigator).connection ?? null, null);
...
export const getSystemInfo = async (): Promise<SystemInfo> => {
    ...
    if (nav.userAgentData) {
        return _parseFromUserAgentData(nav.userAgentData);
    }
    ...
    try {
        ...
        if (gl && gl instanceof WebGLRenderingContext) {
            ...
            if (debugInfo) {
                ...
                if (renderer && typeof renderer === 'string' && renderer.toLowerCase().includes("adreno")) {
                    vendor = "Qualcomm";
                }
